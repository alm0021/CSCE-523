...
public class MinimaxAlphaBetaSearch<BOARD extends Board, MOVE extends Move> implements Search<BOARD, MOVE> {
  ...
  final double MIN = -100000.0;
  final double MAX = 100000.0;
  ...
  public MOVE findBestMove(BOARD board, int depth) {
    ...
    while (i <= depth) {
      ...
      System.out.println("Depth: " + i + " Time: " + elapsedTime / 1000.0 + " " + currentPeriod / 1000.0
          + " Nodes Searched: " + totalNodesSearched + " Leaf Nodes: " + totalLeafNodes + " Rate: " + rate);
  ...
  private MOVE Minimax(int depth) {
    ...
    while (movesList != null) {
      ...
      double theMoveVal = selectMinimax(depth, MIN, MAX);
      // Undo move
   ...
   */
  public double selectMinimax(int depth, double alpha, double beta) {
    int score = gameOver();
    ...
    if (this.board.getCurrentPlayer() == 1) {
      ...
      while (maxMoves != null) {
        ...
        best = Math.max(best, selectMinimax(depth + 1, alpha, beta));
    ...
    // If player is WHITE, select MIN value Move
      ...
      while (minMoves != null) {
        ...
        totalNodesSearched++;
        // Use Minimax to get max value
        best = Math.min(best, selectMinimax(depth + 1, alpha, beta));
